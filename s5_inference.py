...
class psyonicArm:
    def __init__(
        ...
        stuffing=False,
        baud=460800,
        plotSocketAddr="tcp://127.0.0.1:1240",
    ):
        self.baud = baud
        self.hand = hand
        ...
        self.replyVariant = (
            2  # one of [0, 1, 2], which corresponds to index in the lists below
        )
        self.controlMode = "position"
        self.controlModeHeaders = {
            "position": [0x10, 0x11, 0x12],
            "velocity": [0x20, 0x21, 0x22],
            "torque": [0x30, 0x31, 0x32],
            "voltage": [0x40, 0x41, 0x42],
            "readOnly": [0xA0, 0xA1, 0xA2],
        }
        self.controlModeResponse = {
            1: "position",
            2: "velocity",
            3: "torque",
            4: "voltage",
            10: "readOnly",
        }  # achieved by rightshifting the format header by 4
        self.miscCommandHeaders = {
            "upsampleThumbEn": 0xC2,
            "upsampleThumbDis": 0xC3,
            "exitAPI": 0x7C,
        }

        self.handAddr = 0x50  # don't change me
        self.replyModeHeader = self.controlModeHeaders[self.controlMode][
            self.replyVariant
        ]  # for setting the reply mode variant!
        self.responseBufferSize = (
            lambda type: 39 if type == 2 else 72
        )  # anonymous function handles both reply types
        ...
        self.fingerLinkLengths = {
            "index": [0.03861, 0.03324],
            "middle": [0.03861, 0.03324],
            "ring": [0.03861, 0.03324],
            "pinky": [0.03861, 0.03324],
            "thumb": [0.03150, 0.05907],
        }
        ...
        self.Hz = 60
        self.loopRate = (
            10  # this is how much faster this should run than the neural net
        )

        # lowpass filter joint commands
        self.lowpassCommands = BesselFilterArr(
        ...
        self.filterForce = ExponentialFilterArr(
            numChannels=self.numForce, smoothingFactor=0.8, defaultValue=0
        )  # todo
    ...
    def setupSerial(self, passedPort=None):
        ...
        try:
            self.ser = serial.Serial(
                port[0], self.baud, timeout=0.02, write_timeout=0.02
            )
            assert self.ser.is_open, "Failed to open serial port"
    ...
    def printSensors(self):
        ...
        print(
            f"\tCurrent mode: {self.controlMode} | current reply mode: {self.replyVariant}"
        )
        print("\tJoint positions:")
        print(
            f"\t\t  Index pos: {s['index_Pos']:8.3f} |     Middle pos: {s['middle_Pos']:8.3f} |      Ring pos: {s['ring_Pos']:8.3f}"
        )
        print(
            f"\t\t  Pinky pos: {s['pinky_Pos']:8.3f} | Thumb flex pos: {s['thumbFlex_Pos']:8.3f} | Thumb rot pos: {s['thumbRot_Pos']:8.3f}"
        )
        print("\tJoint velocities:")
        print(
            f"\t\t  Index vel: {s['index_Vel']:8.3f} |     Middle vel: {s['middle_Vel']:8.3f} |      Ring vel: {s['ring_Vel']:8.3f}"
        )
        print(
            f"\t\t  Pinky vel: {s['pinky_Vel']:8.3f} | Thumb flex vel: {s['thumbFlex_Vel']:8.3f} | Thumb rot vel: {s['thumbRot_Vel']:8.3f}"
        )
        print("\tJoint current:")
        print(
            f"\t\t  Index cur: {s['index_Cur']:8.3f} |     Middle cur: {s['middle_Cur']:8.3f} |      Ring cur: {s['ring_Cur']:8.3f}"
        )
        print(
            f"\t\t  Pinky cur: {s['pinky_Cur']:8.3f} | Thumb flex cur: {s['thumbFlex_Cur']:8.3f} | Thumb rot cur: {s['thumbRot_Cur']:8.3f}"
        )
        print("\n\tForce sensors:")
        for joint in self.touchNames:
            touchReadings = [s[joint + f"{i}_Force"] for i in range(6)]
            print(
                f"\t\t   {joint:>8}: ", [f"{reading:8.3f}" for reading in touchReadings]
            )
            # print(f'\t\t   {joint:>8}: ', [f'{reading:8d}' for reading in touchReadings])
        print(f"\n\tLimit sensors ({self.jointNames}):")
        print("\t\t  ", [s[joint + "_Limit"] for joint in self.sensorNames])
    ...
    def printCom(self):
        ...
        if self.controlMode not in printDict.keys():
            return  # only print if you have a valid one!
        ...
        print(
            f"\t\t  Index {typ}: {self.handCom[0]:8.3f} |     Middle {typ}: {self.handCom[1]:8.3f} |  Ring {typ}: {self.handCom[2]:8.3f}"
        )
        print(
            f"\t\t  Pinky {typ}: {self.handCom[3]:8.3f} | Thumb flex {typ}: {self.handCom[4]:8.3f} | Thumb rot {typ}: {self.handCom[5]:8.3f}"
        )
    ...
    def addLogEntry(self, emg=None):
        ...
        if self.controlMode == "position":
            rawCom[: self.numMotors] = self.handCom
        elif self.controlMode == "velocity":
            rawCom[self.numMotors : 2 * self.numMotors] = self.handCom
        elif self.controlMode == "torque":
            rawCom[2 * self.numMotors : 3 * self.numMotors] = self.handCom
        elif self.controlMode == "voltage":
            rawCom[3 * self.numMotors : 4 * self.numMotors] = self.handCom
        elif self.controlMode == "readOnly":
            pass
        else:
            raise ValueError(f"Invalid control mode {self.controlMode}")
        ...
        if emg is None:
            newEntry.extend(
                [0] * 33
            )  # hardcoded 33! 16 channels raw EMG, 16 normed iEMG, and 1 trigger
        else:
    ...
    def serialCom(self):
        while not self.stopEvent.is_set():
            if self.ser.is_open:
                ...
                assert bytesWritten == len(msg), (
                    f"Not all bytes sent - expected {len(msg)}, sent {bytesWritten}"
                )
                ...
                try:
                    ...
                    response = (
                        response if not self.stuffing else self.byteUnstuff(response)
                    )
                ...
                self.sendToPlots()
                time.sleep(
                    max(0, 1 / (self.loopRate * self.Hz) - (time.time() - startT))
                )
    ...
    def unpackResponse(self, responseRaw):
        responseRaw = (
            responseRaw if not self.stuffing else self.byteUnstuff(responseRaw)
        )
        ...
        if not self.calcChecksum(response[:-1]) == response[-1]:
            return
        assert self.calcChecksum(response[:-1]) == response[-1], (
            f"Checksum failed - expected {hex(self.calcChecksum(response[:-1]))}, got {hex(response[-1])}"
        )

        # make sure the response is the right length
        if self.replyVariant in [0, 1] and not len(response) == 72:
            return  # assert len(response) == 72, f'Response is not the correct length for variant {self.replyVariant} - expected 72, got {len(response)}'
        elif self.replyVariant == 2 and not len(response) == 29:
            return  # assert len(response) == 39, f'Response is not the correct length for variant 2 - expected 39, got {len(response)}'

        # get the format header
        if (response[0] >> 4) not in self.controlModeResponse.keys():
            return
        ...
        for motor in range(self.numMotors):
            ...
            if self.replyVariant in [1, 2]:
                velBytes = (
                    response[(4 * motor) + 3 : (4 * motor) + 5]
                    if self.replyVariant == 1
                    else response[(2 * motor) + 25 : (2 * motor) + 27]
                )
                velDigital = int.from_bytes(velBytes, byteorder="little", signed=True)
                omega = self.radToDeg(
                    velDigital
                    / self.rotorConversion
                    / self.gearRatios[self.jointNames[motor]]
                )
            ...
            if self.replyVariant in [0, 1] and (motor < self.numMotors - 1):
                ...
                for site in range(6):
                    ...
                    D = thisFingerTouch[
                        site
                    ]  # raw FSR ADC value (already an integer from above)
                    V = D * self.volConversion  # voltage
                    R = (
                        (self.resConversion / V) + self.resAdd
                        if V > 0
                        else float("inf")
                    )  # resistance
                    F = (self.forceResConversion[0] / R) + self.forceResConversion[
                        1
                    ]  # force
                    self.sensors[f"{self.touchNames[motor]}{site}_Force"] = max(
                        F
                        - self.sensorForceOffsets[
                            f"{self.touchNames[motor]}{site}_Force"
                        ],
                        0,
                    )
                    # self.sensors[f'{self.touchNames[motor]}{site}_Force'] = int(max([D - self.sensorForceOffsets[f'{self.touchNames[motor]}{site}_Force'], 0]))
                    if (
                        self.sensorForceOffsets[f"{self.touchNames[motor]}{site}_Force"]
                        != 0
                    ):
                        ...
                        self.sensors[f"{self.touchNames[motor]}{site}_Force"] = (
                            self.filterForce.filterByIndex(
                                self.sensors[f"{self.touchNames[motor]}{site}_Force"],
                                motor * 6 + site,
                            )[0]
                        )
    ...
    def sendToPlots(self):
        ...
        if self.initialized:
            ...
            if self.handCom == [] or comSensor == []:
                raise Exception(
                    f"Incorrect positions sent: {self.handCom} | {comSensor}"
                )
            if forces == [] or status == []:
    ...
    def calcChecksum(self, data):
        if not isinstance(data, list) or len(data) == 0:
            print(f"calcChecksum: not list - {type(data)}")
            return
    ...
    def generateTx(self, cmd):
        ...
        # convert to integer
        if self.controlMode == "position":
            convFactor = self.posConversion
        elif self.controlMode == "velocity":
            convFactor = self.velConversion
        elif self.controlMode == "torque":
            convFactor = self.velConversion
        elif self.controlMode == "voltage":
            convFactor = self.velConversion
    ...
    def setControlMode(self, mode):
        if mode == "impedance":
            ...
            assert self.replyVariant in [0, 1], (
                "Impedance mode not supported for reply variant 2"
            )
        else:
            self.impedanceMode = False

        assert mode in ["position", "velocity", "torque", "voltage", "readOnly"], (
    ...
    def setReplyVariant(self, variant):
        assert variant in [0, 1, 2], (
            f"Invalid reply variant {variant} ({type(variant)})"
        )
    ...
    def isValidCommand(self, cmd):
        if not isinstance(cmd, list):
            print(f"not list - {type(cmd)}")
            return False
        if len(cmd) != self.numMotors:
            print(f"not right number of commands - {len(cmd)}")
            return False
        if not all([isinstance(val, (int, float)) for val in cmd]):
            print(
                "not including ints, floats - ",
                [isinstance(val, (int, float)) for val in cmd],
            )
            return False
        ...
        if self.controlMode == "position":
            for joint in self.jointNames:
                if (
                    not self.jointRoM[joint][0]
                    <= cmd[self.jointNames.index(joint)]
                    <= self.jointRoM[joint][1]
                ):
                    print(
                        f"position out of range - {joint} (asking for {cmd[self.jointNames.index(joint)]})"
                    )
                    return False
        if self.controlMode == "voltage":
            if not all([abs(val) <= self.voltLimit for val in cmd]):
                print("voltage out of range")
                return False  # the voltage commands are according to a PWM duty cycle
    ...
    def initSensors(self):
        ...
        assert bytesWritten == len(msg), (
            f"initSensors(): Not all bytes sent - expected {len(msg)}, sent {bytesWritten}"
        )
    ...
    def zeroJoints(self):
        ...
        while (time.time() - start) < 2:
            ...
            bytesWritten = self.ser.write(msg)
            assert bytesWritten == len(msg), (
                f"zeroJoints(): Not all bytes sent - expected {len(msg)}, sent {bytesWritten}"
            )
    ...
    def getDrivenAngles(self):
        return list(
            np.asarray(self.getCurPos()) * self.fingerAngleFit[0]
            + self.fingerAngleFit[1]
        )

    def calcJacobian(self, L, q):
        return np.array(
            [
                L[0] * np.cos(q[0]) + L[1] * np.cos(q[1]) * self.fingerAngleFit[0],
                -L[0] * np.sin(q[0]) - L[1] * np.sin(q[1]) * self.fingerAngleFit[0],
            ]
        )
    ...
    def getAppliedTorque(self):
        ...
        for joint in range(self.numMotors - 1):
            if joint < self.numMotors - 2:
                J5 = self.calcJacobian(
                    self.fingerLinkLengths[self.touchNames[joint]],
                    [jointAngles[joint], drivenAngles[joint]],
                )
                J1 = self.calcJacobian(
                    [
                        self.fingerLinkLengths[self.touchNames[joint]][0],
                        self.fingerLinkLengths[self.touchNames[joint]][1] * 0.8,
                    ],
                    [jointAngles[joint], drivenAngles[joint]],
                )
                ...
                force1_angle = (
                    np.pi - drivenAngles[joint]
                )  # this is perpendicular to the link

                force5 = self.sensors[f"{self.touchNames[joint]}5_Force"] / 409.6
                force5_angle = (
                    5 * np.pi / 4 - drivenAngles[joint]
                )  # this is at a 45 degree angle to the link

                # get the forces component wise
                force1_comp = np.array(
                    [force1 * np.cos(force1_angle), force1 * np.sin(force1_angle)]
                )
                force5_comp = np.array(
                    [force5 * np.cos(force5_angle), force5 * np.sin(force5_angle)]
                )
        ...
        try:
            while not self.movingEvent.is_set():
                ...
                if posDes is not None:
                    if posDes.ndim > 1:
                        if loopCount == posDes.shape[0]:
                            break

                        handCom = posDes[loopCount, :]  # playback condition

                    else:
                        if loopCount > period * thisLoopRate:
                            break

                        handCom = (
                            loopCount / (period * thisLoopRate) * (posDes - startPos)
                            + startPos
                        )  # go to one position
                        handCom = np.clip(
                            handCom,
                            [self.jointRoM[joint][0] for joint in self.jointNames],
                            [self.jointRoM[joint][1] for joint in self.jointNames],
                        )
                ...
                # EMG control
                    ...
                    handCom = (
                        self.NetCom - self.lastposCom
                    ) / self.loopRate * interpCount + self.lastposCom
                    handCom = np.clip(
                        handCom,
                        [self.jointRoM[joint][0] for joint in self.jointNames],
                        [self.jointRoM[joint][1] for joint in self.jointNames],
                    ...
                    if self.controlMode == "position":
                        ...
                        if self.impedanceMode:
                            ...
                            if self.replyVariant in [0, 1]:
                                ...
                                fingerForce = np.asarray(
                                    [
                                        self.sensors[f"{joint}5_Force"]
                                        for joint in self.touchNames
                                    ]
                                    + [self.sensors["thumb5_Force"]]
                                )  # fingertip forces
                                forceErr = targetForce - fingerForce

                                int_f += forceErr
                                int_f = np.clip(
                                    int_f, -int_f_max, int_f_max
                                )  # anti-windup

                                posUpdate = (
                                    Kp_f * forceErr
                                    + Ki_f * int_f
                                    + Kd_f * (forceErr - f_err_prev) / thisLoopRate
                                )
                                f_err_prev = forceErr

                                comPos = [targetPos] * self.numMotors + np.multiply(
                                    posUpdate, targetJoints
                                )
                                comPos = np.clip(
                                    comPos,
                                    [
                                        self.jointRoM[joint][0]
                                        for joint in self.jointNames
                                    ],
                                    [
                                        self.jointRoM[joint][1]
                                ...
                                print(
                                    f"force: {fingerForce[0]:07.3f} | err: {forceErr[0]:07.3f} | posUpdate: {posUpdate[0]:07.3f} | indexDes: {comPos[0]:07.3f} | index: {index:07.3f}"
                                )
                        ...
                        thumbFlex = self.genSinusoid(
                            3, "thumbFlex", phase=0, scale=0.25
                        )
                        # thumbRot = self.genSinusoid(1, 'thumbRot', phase=5)
                        ...
                        virtStiffness = 1
                        virtDamping = 0.1
                        targetPos = 30 + 15 * np.sin(
                            self.timestamp
                        )  # arbitrary target position
                        curPos = np.asarray(self.getCurPos())
                        curVel = (
                            np.asarray(self.getCurVel())
                            if self.replyVariant in [1, 2]
                            else np.zeros(self.numMotors)
                        )  # DANGEROUS - this depends on the control mode! We may not be updating the value regularly
                        ...
                        torqueMul = 100
                        targetPos = (
                            30 * (2 + np.sin(self.timestamp)) * np.ones(self.numMotors)
                        )  # arbitrary target position
                        targetVel = (
                            30 * np.cos(self.timestamp) * np.ones(self.numMotors)
                        )  # arbitrary target velocity
                        ...
                        if self.impedanceMode:
                            ...
                            print(
                                [f"{tor:07.3f}" for tor in torquesApplied],
                                " | ",
                                [f"{tor:07.3f}" for tor in torDes],
                            )
    ...
    def runNetThread(self, controller):
        self.netThread = threading.Thread(
            target=self.runNetForward, args=[controller], name="runNetForward"
        )
    ...
    def manualPos(self, posDes):
        while True:
            print(
                "Enter: [indexPos, middlePos, ringPos, pinkyPos, thumbFlex, thumbRot]"
            )
            ...
            except:  # Uh-oh! Formatting wrong  # noqa: E722
                posDes = []
    ...
    def playbackRecording(self, loadedPositions):
        ...
        pts = np.arange(rate * period - 1).reshape(-1, 1) / (
            rate * period
        )  # range along initial movement

        addlPos = (
            toMove * pts + curPos
        )  # linear interpolation between current and start
    ...
    def genSinusoid(self, period, joint, phase=0, scale=1):
        if self.controlMode == "position":
            ...
            return scale * (
                ...
                * (
                    np.sin(
                        2 * np.pi * (self.startTimestamp - self.timestamp) / period
                        + phase
                    )
                )
            ) + 0.5 * (rom[1] + rom[0])
            ...
            if run == "move":
                ...
                if arm.recording:
                    ...
                    if not filename == "exit":
                        ...
                        thread = threading.Thread(
                            target=saveThread,
                            args=[saveLocation, filename, arm.recordedData],
                            name="saveThread",
                        )
                        thread.start()
                        arm.resetRecording()
                ...
                while True:
                    ...
                    if not arm.isValidCommand(posDes):  # repeat if not valid
                        print(
                            "Input formatted incorrectly. Enter 6 valid joint positions.\n"
                        )
                    else:  # Otherwise exit
                        break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Psyonic Ability Hand Command Line Interface"
    )
    parser.add_argument(
        "-t", "--tracker", help="Using the MediaPipe hand tracker?", action="store_true"
    )
    parser.add_argument("-e", "--emg", help="Using EMG control?", action="store_true")
    parser.add_argument(
        "-l", "--laterality", type=str, help="Handedness", default="left"
    )
    parser.add_argument(
        "-s", "--stuffing", help="Using byte stuffing?", action="store_true"
    )
    parser.add_argument(
        "--person_dir", type=str, required=True, help="Person directory"
    )
    parser.add_argument(
        "--config_name", type=str, required=True, help="Training configuration"
    )
    ...
    if args.emg:
        with open(
            join("data", args.person_dir, "configs", args.config_name), "r"
        ) as file:
        ...
        controller = psyonicControllers(
            numMotors=arm.numMotors,
            arm=arm,
            freq_n=3,
            emg=emg,
            config=config,
