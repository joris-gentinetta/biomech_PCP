...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--visualize", action="store_true", help="Plot data exploration results"
    )
    parser.add_argument(
        "-hs",
    ...
    parser.add_argument(
        "-e", "--evaluate", action="store_true", help="Evaluate the model"
    )
    ...
    if args.test:  # trains on the training set and saves the test set predictions
        ...
        for set_id, test_set in enumerate(valsets + testsets):
            val_pred = (
                model.predict(test_set, config.features, config.targets)
                .squeeze(0)
                .to("cpu")
                .detach()
                .numpy()
            ...
            test_set.to_parquet(
                join(
                    (data_dirs + test_dirs)[set_id],
                    f"pred_angles-{config.name}.parquet",
                )
            )
            ...
            try:
                model.load(
                    join(
                        ...
                        experiment_name,
                        "models",
                        f"{args.person_dir}-online_{epoch}.pt",
                    )
                )
            except:  # noqa: E722
                break
