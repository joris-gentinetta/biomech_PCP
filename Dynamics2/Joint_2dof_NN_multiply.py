...
class Joint_2dof(Joint):
    ...
    def forward(self, SS, Alphas, dt=0.0166667):
        ...
        # print("K1s[0]", K1s[0])
        for i in range(self.muscle_num):
            K_muscle = K0s[i] + (K1s[i]*(1 + nn_outputs[i][:,0].view(batch_size, 1)))*Alphas[:, i].view(batch_size, 1)
        ...
        B10_11 = 0
        for i in range(self.muscle_num):
            B_F = (K0s[i] + (K1s[i]*(1+ nn_outputs[i][:, 0].view(batch_size, 1)))*Alphas[:, i].view(batch_size, 1)) * \
                (L0s[i] + (L1s[i]*(1 +nn_outputs[i][:, 1].view(batch_size, 1)))* Alphas[:, i].view(batch_size, 1))
            # The following K is respect to w(angle)
            B10_00 += B_F*Ms[i][0]
            B10_11 += B_F*Ms[i][1]
        B10_0 = torch.hstack(
