...
class upperExtremityModel(nn.Module):
    def __init__(self, device, Dynamic_Lr, EMG_mat_Lr, NN_ratio, EMG_Channel_Count=8, muscleType='bilinear', numDoF=4):
        ...
        # self.synergiesLayer = synergies_nn(device=self.device, numDoF=self.numDoF, EMG_Channel_Count=self.EMG_Channel_Count)
    ...
    def forward(self, SS, EMG, dt):
        ...
        return thetas, states, predictions
...
#     """ This class is the fully connected neural network to approximate muscle synergies """
...
#         self.fc1 = nn.Linear(numInputs, numInputs**2).to(self.device)
#         self.fc2 = nn.Linear(numInputs**2, numInputs**3).to(self.device)
#         self.fc3 = nn.Linear(numInputs**3, numInputs**2).to(self.device)
#         self.fc4 = nn.Linear(numInputs**2, numOutputs).to(self.device)
