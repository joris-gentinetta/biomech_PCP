...
def online_train_model():
    with wandb.init():
        ...
        if config.perturb:
            perturb_file = join(
                "data", config.person_dir, "online_trials", "perturb", "perturber.npy"
            )
        else:
            perturb_file = join(
                "data",
        ...
        data_dirs = [
            join("data", config.person_dir, "recordings", recording, "experiments", "1")
        ...
        _, valsets, combined_sets, testsets = get_data(
            config,
            data_dirs,
            args.intact_hand,
            visualize=False,
                ...
                for s in range(config.seq_len):
                    ...
                    y = (
                        torch.stack(
                            [
                                torch.tensor(
                                    angles_history[history_sample], dtype=torch.float32
                                )
                                for history_sample in history_samples
                            ],
                            dim=0,
                    ...
                    if args.keep_states:
                        if config.model_type == "ModularModel":
                            ...
                            for hs in range(len(history_samples) - 1):
                                states_history[history_samples[hs]] = history_states[
                                    :, hs : hs + 1, :
                                ]
