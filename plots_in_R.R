...
create_plot <- function(data) {
  ggplot(data, aes(x = Time, y = Value, color = Color, linetype = Line_Type)) +
    ...
    facet_grid(rows = vars(Recording_Name), cols = vars(Phase), switch = "y") +
    scale_y_continuous(limits = c(-1.3, 1.3), position = "right") +
    # theme_minimal(base_size = 15) + # Cleaner, modern look with larger base size
    theme(
