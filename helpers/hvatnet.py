    ...
    def __init__(self, n_blocks_per_layer=3, n_filters=64, kernel_size=3,
        ...
        for i in range(self.n_layers):
            ...
            conv_blocks = nn.ModuleList(
                [AdvancedConvBlock(n_filters, kernel_size, dilation=dilation) for i in range(n_blocks_per_layer)])
...
class HVATNetv3(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.denoiser = nn.Sequential(
            *[SimpleResBlock(config.n_filters, config.kernel_size) for _ in range(config.n_res_blocks)])
    ...
    def forward(self, x, targets=None):
        ...
        emg_features = outputs[0]  # 200 fps features
...
if __name__ == '__main__':
    hvatnet_v3_params = dict(n_electrodes=8, n_channels_out=8,
                             n_res_blocks=3, n_blocks_per_layer=3,
                             n_filters=128, kernel_size=3,
                             strides=(2, 2, 2), dilation=2,
                             small_strides=(2, 2))
    model = HVATNetv3(config=Config(**hvatnet_v3_params))

    x = torch.randn(8, 8, 120)
    y = model(x)
