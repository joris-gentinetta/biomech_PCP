...
class BesselFilterArr:
    ...
    def __init__(self, numChannels, order, critFreqs, fs, filtType):
        ...
        if filtType not in ["bandstop", "lowpass", "highpass"]:
            raise ValueError(
                f"Invalid filter type {filtType} provided (options are 'lowpass’, ‘highpass’, ‘bandpass’, ‘bandstop’)."
            )
        ...
        if type(critFreqs) is int:
            ...
            len(critFreqs) == 2 and not numChannels == 2
        ):  # a 2 element array for a bandstop, repeat it
            freqs = [critFreqs for _ in range(numChannels)]
        else:
            freqs = critFreqs

        assert len(freqs) == numChannels, (
            f"critFreqs input size ({len(freqs)}) does not match number of channels ({numChannels})"
        )

        self.filters = {"sos": [], "zi": []}
        for i in range(self.numChannels):
            filter_sos = signal.bessel(
                N=order, Wn=freqs[i], btype=filtType, output="sos", fs=fs, analog=False
            )
    ...
    def filter(self, sig):
        ...
        for i in range(self.numChannels):
            out, self.filters["zi"][i] = signal.sosfilt(
                self.filters["sos"][i], sig[i, :], zi=self.filters["zi"][i]
            )
            filterOut[i, :] = out
    ...
    def filterByIndex(self, sig, channel):
        ...
        for i in channel:
            out, self.filters["zi"][i] = signal.sosfilt(
                self.filters["sos"][i], sig, zi=self.filters["zi"][i]
            )
            filterOut = out
