...
class mainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.ax = 100; self.ay = -1500; self.aw = 2000; self.ah = 1200
        ...
        self.powerLineFilters = BesselFilterArr(numChannels=self.emgChannels, order=4, critFreqs=[58, 62], fs=self.sampleRate, filtType='bandstop')
        self.highpassFilters = BesselFilterArr(numChannels=self.emgChannels, order=4, critFreqs=8, fs=self.sampleRate, filtType='highpass')
        self.lowpassFilters = BesselFilterArr(numChannels=self.emgChannels, order=4, critFreqs=3, fs=self.sampleRate, filtType='lowpass')
        ...
        self.ultrasoundThread = threading.Thread(target=self.ultrasoundSignalRecv, name='ultrasoundThread')
    ...
    def dataReadyCallback(self, newData):
        ...
        if self.doesFilter:
            ...
            if self.doesiEMGFilter:
                d[:self.emgChannels] = np.clip(np.abs(d[:self.emgChannels]) - self.noiseLevel[:, None], 0, None)
                d[:self.emgChannels] = np.clip(self.lowpassFilters.filter(d[:self.emgChannels]), 0, None)
                d[:self.emgChannels] = np.divide(d[:self.emgChannels], self.maxVals[:, None])
    ...
    def onRecordBtnClicked(self):
        ...
        if not self.isRecording: # Start recording
            ...
            name = QtWidgets.QFileDialog.getSaveFileName(self, 'Save File', f'EMG_Recording_{time.time()}.csv', 'CSV Files (*.csv)')
            if not name[0] == '':
                saveData = np.asarray(self.recordingBuf)
                savingThread = threading.Thread(target=self.saveThread, name='saveThread', args=[name[0], saveData])
                savingThread.start()
        ...
        while not self.quitEvent.is_set():
            ...
            if not self.isRecording:
                ...
                if not saveStr == '':
                    ...
                    saveData = np.asarray(self.recordingBuf)
                    savingThread = threading.Thread(target=self.saveThread, name='saveThread', args=[f'{saveStr}.csv', saveData])
                    savingThread.start()
...
class normsWindow(QtWidgets.QWidget):
        ...
        def __init__(self, index, parent=None):
            ...
            for row in range(self._p.rows):
                for col in range(self._p.cols):
                    ...
                    textbox.setText(str(self._p.vals[index*self._p.cols*self._p.rows + col*self._p.rows + row]))  # Set default value to 0
    ...
    def __init__(self, parent=None, rows=4, cols=4):
        ...
        self.setGeometry(self._parent.ax + 50, self._parent.ay + 50, self._parent.aw//3, self._parent.ah//3) # open with a constant appropriate offset
    ...
    def saveChanges(self):
        ...
        for tab in range(self.tabs.count()):
            ...
            for row in range(self.rows):
                for col in range(self.cols):
                    thisVal = current_widget.grid.itemAtPosition(row, col).widget().text()

                    try:
                        if tab == 0 and float(thisVal) <= 0: thisVal = '1' # don't let you set the max to 0 or negative! It'll crash!

                        # newVals[tab*self.cols*self.rows + row*self.cols + col] = thisVal
                        newVals[tab*self.cols*self.rows + col*self.rows + row] = thisVal
        ...
        try:
            if failedFlag: raise ValueError
        ...
        for tab in range(self.tabs.count()):
            ...
            for row in range(self.rows):
                for col in range(self.cols):
                    ...
                    textbox.setText(str(self.vals[tab*self.cols*self.rows + col*self.rows + row]))  # reset to default
