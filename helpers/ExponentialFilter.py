...
class ExponentialFilterArr():
    ...
    def __init__(self, numChannels, smoothingFactor, defaultValue=1):
        ...
        if type(smoothingFactor) in [int, float]:
            ...
            raise ValueError(f'Invalid smoothing factor {smoothingFactor} provided. Must be an integer or a list of integers.')
        
        if type(defaultValue) is int:
            self.defaultValue = defaultValue
        else:
            raise ValueError(f'Invalid default value {defaultValue} provided. Must be an integer.')

        assert len(self.alpha) == numChannels, f'smoothingFactor input size ({len(self.alpha)}) does not match number of channels ({numChannels})'
        assert all([0 <= a <= 1 for a in self.alpha]), 'All smoothing factors must be between 0 and 1'
    ...
    def resetFilterByIndex(self, channel):
        if channel >= self.numChannels or channel < 0:
            raise ValueError(f'Invalid channel index {channel} provided. Must be between 0 and {self.numChannels-1}')
        
        self.filters[channel] = self.defaultValue

    def filter(self, sig):
        self.filters = np.multiply(self.alpha, sig) + np.multiply((1. - np.asarray(self.alpha)), self.filters)
    ...
    def filterByIndex(self, sig, channel):
        ...
        self.filters[channel] = self.alpha[channel]*sig + (1-self.alpha[channel])*self.filters[channel]
