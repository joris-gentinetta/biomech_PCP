...
class EMG:
    def __init__(
        ...
        self.offlineData = (
            self.offline_data_gen(offlineData) if offlineData is not None else None
        )
        ...
        if system_name == "Linux":
            self.boundsPath = (
                "/home/haptix/haptix/haptix_controller/handsim/include/scaleFactors.txt"
            )
            self.deltasPath = (
                "/home/haptix/haptix/haptix_controller/handsim/include/deltas.txt"
            )
            self.synergyPath = (
                "/home/haptix/haptix/haptix_controller/handsim/include/synergyMat.csv"
            )
        ...
        self.int_window = 0.05  # sec - 50 ms integration window
        ...
        self.rawHistory = np.zeros(
            (self.numElectrodes, self.numPackets)
        )  # switch to processing packets (apply filters only once on a number of packets)
    ...
    def offline_data_gen(self, data):
        padded = np.pad(
            data, ((0, 0), (self.numPackets * 100, self.numPackets * 100)), mode="edge"
        )
    ...
    def resetEMG(self):
        self.rawEMG = [
            -1
        ] * self.numElectrodes  # this is RAW from the board - need to get iEMG
        self.iEMG = [-1] * self.numElectrodes  # this is iEMG
        self.filtEMG = [
            -1
        ] * self.numElectrodes  # this is the high pass and motion artifcats EMG
        self.normedEMG = [-1] * self.numElectrodes  # array of normalized EMG values
        self.muscleAct = (
            [-1] * self.numElectrodes
        )  # array of muscle activation (through low pass muscle activation dynamics)
        self.prevAct = [
            -1
        ] * self.numElectrodes  # array of previous muscle activation values
        self.synergies = (
            [-1] * self.numSynergies
            if self.usingSynergies
            else [-1] * self.numElectrodes
        )

    def initFilters(self):
        self.powerLineFilterArray = BesselFilterArr(
    ...
    def getRawEMG(self, electrode):
        if electrode >= self.numElectrodes:
            raise ValueError(
                f"getRawEMG(): Asked for invalid electrode {electrode} of {self.numElectrodes}"
            )
    ...
    def getiEMG(self, electrode):
        if electrode >= self.numElectrodes:
            raise ValueError(
                f"getiEMG(): Asked for invalid electrode {electrode} of {self.numElectrodes}"
            )
    ...
    def getFiltEMG(self, electrode):
        if electrode >= self.numElectrodes:
            raise ValueError(
                f"getFiltEMG(): Asked for invalid electrode {electrode} of {self.numElectrodes}"
            )
    ...
    def getNormedEMG(self, electrode):
        if electrode >= self.numElectrodes:
            raise ValueError(
                f"getNormedEMG(): Asked for invalid electrode {electrode} of {self.numElectrodes}"
            )
    ...
    def getFilteredEMG(self, electrode):
        if electrode >= self.numElectrodes:
            raise ValueError(
                f"getFilteredEMG(): Asked for invalid electrode {electrode} of {self.numElectrodes}"
            )
    ...
    def getSynergy(self, synergy):
        if synergy >= self.numSynergies:
            raise ValueError(
                f"getSynergy(): Asked for invalid synergy {synergy} of {self.numSynergies}"
            )
    ...
    def intEMGPacket(self):
        ...
        self.filtEMG = np.copy(
            np.asarray(emg)[:, -1]
        )  # motion artifacts and drift removed
    ...
    def muscleDynamics(self):
        ...
        muscleAct = np.where(
            u > prevA,
            (u * (b + (1 - b) * u) / tauA + prevA / Ts)
            / (1 / Ts + (b + (1 - b) * u) / tauA),
            (u / tauD + prevA / Ts) / (1 / Ts + 1 / tauD),
        )
    ...
    def synergyProd(self):
        if not self.numSynergies == len(self.usedChannels):
            raise ValueError(
                f"synergyProd(): Wrong number of channels ({len(self.usedChannels)} provided; require {self.numSynergies}"
            )
    ...
    def pipelineEMG(self):
        ...
        while not self.exitEvent.is_set():
            ...
            if self.offlineData is not None:
                ...
                self.emgHistory = np.concatenate(
                    (self.emgHistory, self.normedEMG), axis=1
                )
...
if __name__ == "__main__":
    emg_data = np.load(
        "/Users/jg/projects/biomech/DataGen/data/joris/trigger_1/triggered_emg.npy"
    ).T
    emg_timestamps = np.load(
        "/Users/jg/projects/biomech/DataGen/data/joris/trigger_1/triggered_emg_timestamps.npy"
    )
    ...
    filtered_emg_timestamps = [
        emg_timestamps[i * emg.numPackets + emg.numPackets // 2]
    ...
    normalized_emg = np.clip(
        (filtered_emg - min_vals[:, None]) / (max_vals - min_vals)[:, None], 0, 1
    )
