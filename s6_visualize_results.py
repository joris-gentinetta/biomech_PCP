...
def move_finger(hand, finger, angle):
    ...
    p.setJointMotorControl2(
        hand, id1, p.POSITION_CONTROL, targetPosition=angle * multiplier + offset
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name specified in the config file used to train the model",
    )
    ...
    if args.video:
        cap = cv2.VideoCapture(
            join(
                ...
                "visualization_corrected.mp4",
            )
        )
    target_angles = pd.read_parquet(
        join(
            args.data_dir,
            "experiments",
            args.experiment_name,
    ...
    if args.model_name:
        ...
        pred_angles = pd.read_parquet(
            join(
                args.data_dir,
                "experiments",
                args.experiment_name,
                f"pred_angles-{args.model_name}.parquet",
            )
        )
    ...
    camera_target_position = [
        ...
        -0.65,
        -0.3,
    ]  # Focus on the center of your model or a specific part
    ...
    for i in range(target_angles.shape[0]):
        move_finger(
            target_hand, "index", target_angles.loc[i, (args.intact_hand, "indexAng")]
        )
        move_finger(
            target_hand, "middle", target_angles.loc[i, (args.intact_hand, "midAng")]
        )
        move_finger(
            target_hand, "ring", target_angles.loc[i, (args.intact_hand, "ringAng")]
        )
        move_finger(
            target_hand, "pinky", target_angles.loc[i, (args.intact_hand, "pinkyAng")]
        )
        ...
        p.setJointMotorControl2(
            target_hand, joint_ids["thumb"][0], p.POSITION_CONTROL, targetPosition=angle
        )
        angle = target_angles.loc[i, (args.intact_hand, "thumbOutPlaneAng")]
        p.setJointMotorControl2(
            target_hand, joint_ids["thumb"][1], p.POSITION_CONTROL, targetPosition=angle
        )
        ...
        if args.model_name:
            move_finger(
                pred_hand, "index", pred_angles.loc[i, (args.intact_hand, "indexAng")]
            )
            move_finger(
                pred_hand, "middle", pred_angles.loc[i, (args.intact_hand, "midAng")]
            )
            move_finger(
                pred_hand, "ring", pred_angles.loc[i, (args.intact_hand, "ringAng")]
            )
            move_finger(
                pred_hand, "pinky", pred_angles.loc[i, (args.intact_hand, "pinkyAng")]
            )
