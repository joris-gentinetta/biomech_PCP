...
def crop_data(
    cap, data_dir, out_dir, start_frame, end_frame, x_start, x_end, y_start, y_end
):
    ...
    output_path = join(out_dir, "cropped_video.mp4")
    ...
    np.save(
        join(out_dir, "cropped_video_timestamps.npy"),
        video_timestamps[start_frame:end_frame],
    )
...
def trigger_crop_video(data_dir):
    ...
    output_path = join(data_dir, "triggered_video.mp4")
    out = cv2.VideoWriter(
        output_path,
        fourcc,
    ...
    with tqdm(total=int(cap.get(cv2.CAP_PROP_FRAME_COUNT))) as pbar:
        while cap.isOpened():
            ...
            if (
                frame[0, frame.shape[1] // 2, 0] >= 200
                and frame[0, frame.shape[1] // 2, 1] >= 200
                and not found_trigger
            ):
                found_trigger = True
            ...
            if (
                found_trigger
                and frame[0, frame.shape[1] // 2, 0] < 200
                and frame[0, frame.shape[1] // 2, 1] < 200
            ):
                record_frame = True
                found_trigger = False
...
def trigger_crop_emg(cap, data_dir, trigger_channel, trigger_value):
    ...
    for i in range(len(video_timestamps)):
        video_timestamps[i] = emg_timestamps[start_frame] + (
            emg_timestamps[end_frame] - emg_timestamps[start_frame]
        ) * i / len(video_timestamps)
...
def filter_emg(data_dir):
    ...
    emg = EMG(
        samplingFreq=sf, offlineData=emg_data, maxVals=maxVals, noiseLevel=noiseLevel
    )
...
def downsample_video(cap, out_dir):
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    output_path = join(out_dir, "cropped_downsampled_video.mp4")
    out = cv2.VideoWriter(
        output_path,
        fourcc,
        60,
    ...
    np.save(
        join(out_dir, "cropped_video_timestamps.npy"), np.array(new_video_timestamps)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--x_start", type=int, default=0, help="Start x coordinate for cropping"
    )
    parser.add_argument(
        "--x_end", type=int, default=-1, help="End x coordinate for cropping"
    )
    parser.add_argument(
        "--y_start", type=int, default=0, help="Start y coordinate for cropping"
    )
    parser.add_argument(
        "--y_end", type=int, default=-1, help="End y coordinate for cropping"
    )
    parser.add_argument(
        "--start_frame", type=int, default=0, help="Start frame for cropping"
    )
    parser.add_argument(
        "--end_frame", type=int, default=-1, help="End frame for cropping"
    )
    parser.add_argument(
        "--trigger_channel", type=int, required=True, help="Trigger channel"
    )
    parser.add_argument("--trigger_value", type=int, default=550, help="Trigger value")
    parser.add_argument(
        "--process",
        action="store_true",
        help="Process video, otherwise show video and EMG trigger channel",
    )
    ...
    start_frame, end_frame = trigger_crop_emg(
        cap,
        args.data_dir,
        trigger_channel=args.trigger_channel,
        trigger_value=abs(args.trigger_value),
    )
    ...
    if not args.process:
        ...
        x_start, x_end, y_start, y_end, start_frame, end_frame = check_cropping_args(
            args
        )
        crop_data(
            cap,
