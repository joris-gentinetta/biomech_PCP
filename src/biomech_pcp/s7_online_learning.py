...
import wandb
from biomech_pcp.helpers.online_utils import (
    AnglesProcess,
    InputThread,
    JointsProcess,
    ProcessManager,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--intact_hand", type=str, required=True, help="Intact hand (Right/Left)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-s", "--save_model", action="store_true", help="Save a model"
    )  # todo
    parser.add_argument("--offline", action="store_true", help="Offline training")
    parser.add_argument(
        "-v", "--visualize", action="store_true", help="Visualize hand movements"
    )
    parser.add_argument(
        "-en", "--experiment_name", type=str, required=True, help="Experiment name"
    )
    parser.add_argument("-c", "--camera", type=int, required=False, help="Camera id")
    parser.add_argument(
        "-si", "--save_input", action="store_true", help="Save input data"
    )
    parser.add_argument(
        "-cf",
        "--calibration_frames",
    ...
    parser.add_argument(
        "--keep_states", action="store_true", help="Keep states in history"
    )
    parser.add_argument(
        "--jorisThumb", action="store_true", help="Use Joris thumb model"
    )
    ...
    if args.save_input:
        ...
        save_path = join(
            "data", args.person_dir, "online_trials", args.experiment_name, "models"
        )
    ...
    with open(
        join("data", args.person_dir, "configs", f"{args.config_name}.yaml"), "r"
    ) as file:
    ...
    if args.perturb:
        ...
        perturb_file = join(
            "data", args.person_dir, "online_trials", "perturb", "perturber.npy"
        )
    ...
    os.makedirs(
        join("data", args.person_dir, "online_trials", args.experiment_name),
        exist_ok=True,
    )
    ...
    if args.save_input:
        ...
        st = SaveThread(
            vc.outputQ, frame_size=(int(width), int(height)), save_path=save_path
        )
        processManager.manage_process(st)

    elif args.offline:
        data_dirs = [
            join("data", args.person_dir, "recordings", recording, "experiments", "1")
        ...
        test_dirs = (
            [
                join(
                    "data", args.person_dir, "recordings", recording, "experiments", "1"
                )
                for recording in config.test_recordings
            ]
            if config.test_recordings is not None
            else None
        )
        ...
        if config.model_type == "ModularModel":  # todo
            ...
            for param in model.model.muscle_model.parameters():
                param.requires_grad = (
                    False if epoch < config.muscle_model["n_freeze_epochs"] else True
                )
            for param in model.model.joint_model.parameters():
                param.requires_grad = (
                    False if epoch < config.joint_model["n_freeze_epochs"] else True
                )
        ...
        train_dataset = OLDataset(
            trainsets, config.features, config.targets, device=device
        )
        ...
        if args.keep_states:
            ...
            x = np.concatenate(
                [
                    np.concatenate(
                        [
                            np.expand_dims(np.expand_dims(ah, axis=0), axis=1)
                            for ah in angles_history[hs - 1 : hs + 1]
                        ],
                        axis=1,
                    )
                    for hs in history_samples
                ],
                axis=0,
            )
        ...
        with tqdm(range(1, config.n_epochs + 1)) as pbar:
            ...
            for epoch in pbar:
                ...
                for i in range(epoch_len):
                    history_samples = np.random.randint(
                        1, len(angles_history), size=config.batch_size - 1
                    )
                    history_samples = np.array(
                        [history_sample for history_sample in history_samples]
                        + [len(angles_history) - 1]
                    )
                    if args.keep_states:
                        if config.model_type == "ModularModel":
                            states = [
                                ...
                                [
                                    torch.concat(
                                        [
                                            states_history[history_sample][0]
                                    ...
                                    torch.concat(
                                        [
                                            states_history[history_sample][1]
                                            for history_sample in history_samples
                                        ],
                                        dim=0,
                                    ),
                                    torch.concat(
                                        [
                                            states_history[history_sample][2]
                                            for history_sample in history_samples
                                        ],
                                        dim=0,
                                    ),
                        ...
                        x = np.concatenate(
                            [
                                np.concatenate(
                                    [
                                        np.expand_dims(
                                            np.expand_dims(ah, axis=0), axis=1
                                        )
                                        for ah in angles_history[hs - 1 : hs + 1]
                                    ],
                                    axis=1,
                    ...
                    for s in range(config.seq_len):
                        ...
                        x = (
                            torch.stack(
                                [
                                    torch.tensor(
                                        emg_history[history_sample], dtype=torch.float32
                                    )
                                    for history_sample in history_samples
                                ],
                                dim=0,
                        ...
                        if args.visualize and not visualizeQueue.full():
                            ...
                            pred_angels_df.loc[config.targets] = (
                                outputs[-1].squeeze().to("cpu").detach().numpy()
                            )
                        ...
                        if args.keep_states:
                            if config.model_type == "ModularModel":
                                history_states = [
                                    states[2][st].detach() for st in range(3)
                                ]
                                ...
                                for hs in range(len(history_samples) - 1):
                                    states_history[history_samples[hs]] = [
                                        history_states[0][hs : hs + 1],
                                        history_states[1][hs : hs + 1],
                                        history_states[2][hs : hs + 1],
                                    ]
                ...
                pbar.set_postfix(
                    {"lr": lr, "train_loss": epoch_loss, "val_loss": val_loss}
                )
        ...
        # get_data(config, data_dirs, args.intact_hand, visualize=False,
        #                                               test_dirs=test_dirs)
        ...
        if config.model_type == "ModularModel":  # todo
            epoch = 0
            # for param in model.model.activation_model.parameters():
            #     param.requires_grad = False if epoch <
            # config.activation_model['n_freeze_epochs'] else True
            for param in model.model.muscle_model.parameters():
                param.requires_grad = (
                    False if epoch < config.muscle_model["n_freeze_epochs"] else True
                )
        ...
        anglesProcess = AnglesProcess(
            args.intact_hand, queue_size, joint_to_angles_Q, joris=args.jorisThumb
        )
        jointsProcess = JointsProcess(
            args.intact_hand,
            queue_size,
            joint_to_angles_Q,
            save_path,
        ...
        if args.keep_states:
            # states = model.model.get_starting_states(1,
            # torch.tensor(angles_history[-2:],
        ...
        with tqdm(range(1, config.n_epochs + 1)) as pbar:
            for epoch in pbar:
                ...
                epoch_end_time = time()
                print(
                    "average epoch fps: ", counter / (epoch_end_time - epoch_start_time)
                )
