...
class psyonicArm():
	def __init__(self, hand='right', usingEMG=False, stuffing=False, baud=460800, plotSocketAddr='tcp://127.0.0.1:1240'):
		...
		self.filterForce = ExponentialFilterArr(numChannels=self.numForce, smoothingFactor=0.8, defaultValue=0)
	...
	def unpackResponse(self, responseRaw):
		...
		# elif self.replyVariant == 2 and not len(response) == 29: print(f'Response is not the correct length for variant 2 - expected 39, got {len(response)}')
		
		# get the format header
		if not (response[0] >> 4) in self.controlModeResponse.keys(): print(response[0]); return
			...
			while not self.movingEvent.is_set():
				...
				# sinusoidal control
					...
					if self.controlMode == 'position':
						...
						index = self.genSinusoid(1, 'index', phase=0)
						middle = self.genSinusoid(1, 'middle', phase=1)
						ring = self.genSinusoid(1, 'ring', phase=2)
						pinky = self.genSinusoid(1, 'pinky', phase=3)
