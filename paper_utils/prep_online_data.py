...
def load_and_split_file(file_path, length=None):
    ...
    if file_path.endswith(".npy"):
        ...
        return data.iloc[:midpoint].reset_index(drop=True), data.iloc[
            midpoint:length
...
def concatenate_files(data_dir, folders, experiment):
    ...
    for folder in folders:
        ...
        if emg_file_path.exists() and angles_file_path.exists():
            ...
            angles_first_half, angles_second_half = load_and_split_file(
                angles_file_path, length
            )
    ...
    final_angles_first_half = pd.concat(
        [final_angles_first_half, final_angles_second_half], ignore_index=True
    )
    np.save(
        output_dir / "cropped_emg.npy",
        np.concatenate([final_emg_first_half, final_emg_second_half], axis=0),
    )
...
if __name__ == "__main__":
    ...
    file_path = Path(
        f"data/{args.person_id}/recordings/online_concat_comp/experiments/1"
    )
    output_file_path = Path(
        f"data/{args.person_id}/recordings/online_concat_comp_interp/experiments/1"
    )
